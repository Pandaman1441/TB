...
func _would_cross_existing_path(i:int, j:int, room: Room) -> bool:
	...
	if right_neighbour and room.col> j:
		for next_room: Room in right_neighbour.next_rooms:
			if next_room.col < room. col:
				return true
	if left_neighbour and room.col < j:
...
func _room_has_parent_of_type(room: Room, type: Room.Type) -> bool:
	...
	if room.col > 0 and room.row >0:
		var parent_candidate := map_data[room.row-1][room.col -1] as Room
		if parent_candidate.next_rooms.has(room):
			parents.append(parent_candidate)
	if room.row > 0:
		var parent_candidate := map_data[room.row-1][room.col] as Room
		if parent_candidate.next_rooms.has(room):
			parents.append(parent_candidate)
	if room.col < MAP_WIDTH-1 and room.row > 0:
		var parent_candidate := map_data[room.row-1][room.col+1] as Room
		if parent_candidate.next_rooms.has(room):
			parents.append(parent_candidate)
